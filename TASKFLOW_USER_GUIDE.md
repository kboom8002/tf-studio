...
1. 브라우저에서 `localhost:3000/register` 메뉴로 진입하여 이메일(예: `kboom8002@gmail.com`)과 **본인이 사용할 임의의 비밀번호**를 입력해 가입합니다.
2. 가입 즉시 **AUTHOR(작성자)** 권한으로 시스템에 다이브(Dive)하게 되며, 상단 우측 GNB에 내 권한 배지가 나타납니다.
(이때 설정하신 비밀번호가 향후 로그인 시 사용될 본인의 고유 암호입니다.)

### Step 5. 최고 관리자 (Super Admin) 권한 획득 (백도어)
시스템의 룰을 제어하기 위해 본인 계정을 최고 관리자로 승급시켜야 합니다. (별도의 공용 어드민 계정/비밀번호가 있는 것이 아니라, 내 계정의 "등급"을 올리는 방식입니다.)
